...
void loop() {
  ...
  //if(testMode){
  //int jfc = 0;
  /*if((pauseRot == true) && (jfc == 0)){
    //pauseRot = false;
    //jfc = 1;
    resetValues();
    //desiredXPos = 48;
    //desiredCounts = (desiredXPos / (2.0 * PI*radius)) * N;
  ...
  int jfc = 0;
    switch(state){
      case 1:
        // rotate in place slowly until marker found
        velController(leftInitialRot, rightInitialRot, L_angVel, R_angVel,velK);
        // reset encoder counts
        //    reset angular positions
        // set desireRotatAngPos
        // set desireForwardAngPos
        // set state to correctAngle
        ...
        break;
 /*//     case 2:
 //       // use rotPIController to angle robot towards aruco
 //       rotPiController(desireRotatAngPos, R_angPosNow, L_angPosNow, rotK);
 //       // reset encoder counts
 //       //    reset angular positions
 //       // reset integral errors
 //       
 //       break;*/
      case 2:
        // use motorPIController to drive robot 1 foot away from aruco
        motorPiController(desireForwardAngPos, desireForwardAngPos, L_angPosNow, R_angPosNow, linK);
        // reset encoder counts
        //    reset angular positions
        // reset integral errors
        ...
        break;
      case 3:
        // rotate robot in place until perpendicular to aruco
...
void rotPiController(double desiredPhi, double rightAngNow, double leftAngNow, double rotK[4]) {
  ...
  /*if (leftrotError < rotErrorRange) {                                     // Check if left wheel within acceptable error range
    ...
    if (numRotFlag >= 2) {                                                // more robust controls for future demos / projects
      ...
      R_countNow = 0;                                                     // reset global variables to zero for forward motion
      L_countNow = 0; 
...
void printData() {
  ...
  //Serial.print("\t"); Serial.print(pauseRot);
  //Serial.print("\t"); Serial.print(isrCalls);
  //Serial.print("\t"); Serial.print(i2cDistance);
